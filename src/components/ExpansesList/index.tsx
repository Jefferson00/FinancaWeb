...
const ExpansesList = observer(() => {
  ...
  useEffect(() => {
    const censoredStatusStoraged = localStorage.getItem(`financaWeb.censored.expanseList`);
            ...
            return (
              <div key={index}>
                ...
                {item.items.map((i, index) => {
                  return (
                    <ItemView key={index} type={'EXPANSE'} item={i} />
                  )
                })}
              </div>
            )
